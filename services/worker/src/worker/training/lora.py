...
def run(context: TrainingExecutionContext) -> TrainingAlgorithmResult:
    ...
    output_dir = resolve_output_dir("lora", context["experiment_name"])
    ...
    return TrainingAlgorithmResult(
        metrics=metrics,
        artifacts={"trainable_params": trainable, "lora_r": _LORA_R, "lora_alpha": _LORA_ALPHA, "checkpoint_dir": output_dir},
    )
