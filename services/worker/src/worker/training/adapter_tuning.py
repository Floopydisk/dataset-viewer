...
"""Adapter Tuning (IA³): scale keys, values, and FFN activations with tiny learned vectors."""
...
def _load_adapter_model(model_name: str, task_type: str) -> tuple[Any, int]:
    ...
    logging.info(f"Adapter tuning IA³ ({task_type}): {trainable:,} / {total:,} parameters trainable")
...
def run(context: TrainingExecutionContext) -> TrainingAlgorithmResult:
    ...
    output_dir = resolve_output_dir("adapter-tuning", context["experiment_name"])
    ...
    return TrainingAlgorithmResult(
        metrics=metrics,
        artifacts={"trainable_params": trainable, "adapter_type": "ia3", "checkpoint_dir": output_dir},
    )
