...
"""Prompt Tuning: prepend trainable soft prompt tokens only at the input embedding layer."""
...
_NUM_VIRTUAL_TOKENS = 8
# PromptTuningConfig has reliable PEFT support only for generative task types
_SUPPORTED_TASK_TYPES = frozenset(["seq2seq", "summarization", "causal-lm", "text-classification"])


def _load_prompt_model(model_name: str, task_type: str) -> tuple[Any, int]:
    if task_type not in _SUPPORTED_TASK_TYPES:
        raise ValueError(
            f"Prompt tuning does not support task_type '{task_type}'. "
    ...
    logging.info(f"Prompt tuning ({task_type}): {trainable:,} / {total:,} parameters trainable")
...
def run(context: TrainingExecutionContext) -> TrainingAlgorithmResult:
    ...
    output_dir = resolve_output_dir("prompt-tuning", context["experiment_name"], run_id=context["job_id"])
